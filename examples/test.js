...
client.init(options, function(err){
  ...
  client.connect(function(err){
    ...
    client.writeFile("./testdir/testfile.txt", "bloddyblaaaaaah", function(err) {
      if (err)
        console.log(err);
    });    
  
    client.readFile("./testdir/testfile.txt", function(err, data){
      if (err)
        console.log(err);
      console.log(data.toString());      
    });
    
    client.readdir("./testdir", function(err, dirs){
        console.log(dirs);
    });
  
    client.readFile("./testdir/wargarbl", function(err, data){
      console.log(err);      
    });
    
    client.stat("./testdir/testfile.txt", function(err, stat){
      console.log(err);
      console.log(stat.isDirectory());    
    });
    
    client.exec("ls", function(err, data) {
      console.log(1);  
    });
    ...
    client.exec("ls testdir", function(err, data) {
      console.log(data);  
    });
    
    client.exec("rm -r testdir", function(err, data) {
      if (err)
        console.log(err);  
    });
  });    
